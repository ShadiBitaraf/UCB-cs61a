...
def scheme_read(src):
    # a Buffer instance
    # removes enough from src to form and expression and returns it
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 2
        rest = read_tail(src)  # rest of src
        return rest
        # END PROBLEM 2
    elif val == "'":
        # BEGIN PROBLEM 3
...
def read_tail(src):
    # reads the rest of scheme_read.
    # gotta call scheme-read until it hits the matching '('
    # returns a linked list of Pair instances of expressions.
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            para1 = scheme_read(src)
            para2 = read_tail(src)
            return Pair(para1, para2)
