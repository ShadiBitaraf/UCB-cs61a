...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            added_ant = self
            if original_ant.can_contain(added_ant) == True:
                place.ant = original_ant
                original_ant.ant_contained = added_ant
            elif added_ant.can_contain(original_ant) == True:
                place.ant = added_ant
                added_ant.ant_contained = original_ant
            else:
    ...
    def buff(self):
        ...
        # BEGIN Problem 12
        get_ant = self.place.ant

        if self.is_buffed != True:
            get_ant.damage *= 2
            self.is_buffed = True
        if get_ant.is_container == True and get_ant.ant_contained != None:
            if get_ant.ant_contained.is_buffed != True:
                get_ant.ant_contained.damage *= 2
                get_ant.ant_contained.is_buffed = True
        else:
            return None
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.is_hive == False:
            if self.min_range <= num_transitions <= self.max_range:
                if random_bee(current_place.bees) != None:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.active_queen == False:
            gamestate.active_queen = True
