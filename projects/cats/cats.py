...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_list = [elements for elements in paragraphs if select(elements)]
    if k<len(new_list):
        return new_list[k]
    else:
        return ''
...
def about(topic):
    ...
    def helper(paragraphs):
        new_list = [elements for elements in split(remove_punctuation(lower(paragraphs))) if elements in topic]
        if new_list != []:
            return True
        else:
            return False
...
def accuracy(typed, reference):
    ...
    if reference_words == typed_words:
        ...
        # we make a new list, with the extra elements chopped off with [:]
        typed_words = typed_words[:len(reference_words)]

    for i in range(len(typed_words)):
        if reference_words[i] != typed_words[i]:
            incorrect += 1
    return (len_final-incorrect)/(len_final)*100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    elif min([diff_function(typed_word, elements, limit) for elements in valid_words]) > limit:
        return typed_word
    else:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            new_list = new_list + [sofar[i]]
        else: 
            break 
    progress = len(new_list)/len(prompt) 
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    # new_list = [next_element - elements for elements in times_per_player]
    times = []
    new_list = []
    for i in range(len(times_per_player)):
        for j in range(1,len(times_per_player[i])):
            new_list = new_list + [times_per_player[i][j]-times_per_player[i][j-1]]
        times = times + [new_list]
        new_list = []
...
def fastest_words(match):
    ...
    for i in word_indices:
        smallest_t_index= 0
        for player in range(1, len(get_times(match))):
            if time(match, player, i)<time(match, smallest_t_index, i):
                smallest_t_index = player

        final_list[smallest_t_index].append(word_at(match, i))
    return final_list
    # END PROBLEM 10
